...
export default function TR() {
  ...
  return (
    <LessonLayout title="Etiqueta &lt;tr&gt;">
      <div className="lesson-content-panel bg-blanco w-full min-h-[500px] rounded-md shadow-md p-6 sm:p-10 flex flex-col flex-1">
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-10 flex-1 min-w-0">
          <div className="flex flex-col text-lg sm:text-xl lg:text-2xl max-w-2xl">
            <p>
              La etiqueta <span className="font-bold text-amarillo">&lt;tr&gt;</span> es fundamental para la
              creación de tablas, ya que estas representan las filas, es el diminutivo de Table Row.
            </p>
          </div>
...
<table border="1">
<thead>
  `}<span className="text-amarillo">{`<tr>`}</span>{`
    <th>Encabezado 1</th>
    <th>Encabezado 2</th>
  `}<span className="text-amarillo">{`</tr>`}</span>{`
</thead>
<tbody>
  ...
  `}<span className="text-amarillo">{`</tr>`}</span>{`
        ...
        <div className="flex justify-end mt-auto pt-4">
          <button
            onClick={() => navigate("/lessons/lesson1/TR_more")}
